...
This is a powerful optimization technique that can be used to solve problems involving linear recurrences, such as calculating Fibonacci numbers, in logarithmic time.

```cpp
// pseudo code

Matrix m_exp(Matrix base, int exp) {
    if (exp == 0) {
        return identity_matrix;
    }

    Matrix half = m_exp(base, exp / 2);
